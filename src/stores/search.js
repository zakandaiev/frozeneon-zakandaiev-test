...
const useSearchStore = defineStore('search', () => {
  ...
  async function searchByKeyword(options = {}) {
    ...
    debounceTimeout = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`${SETTING.api.registry}/search?text=${keyword.value}&size=${SETTING.search.pagination_limit || 10}&from=${page.value}&quality=1`, options);
    ...
    }, (SETTING.search.debounce_ms || 1000));
  }
  ...
  function assignHistory(data) {
    ...
    if (!isKeywordExists() || data.total !== history.value[keyword.value].total) {
      history.value[keyword.value] = {
        total: data.total,
        pagesCount: Math.ceil(data.total / SETTING.search.pagination_limit),
      };
    }
